...
if (process.env.DATABASE_URL) {
  ...
  db = {
    ...
    async init() {
      ...
      await pool.query(`CREATE TABLE IF NOT EXISTS clicks (
        ...
        user_agent TEXT, fingerprint TEXT, timestamp TIMESTAMP DEFAULT NOW()
      )`);
      ...
      const cmigs = { client_lat: 'DOUBLE PRECISION', client_lng: 'DOUBLE PRECISION', fingerprint: 'TEXT' };
  ...
  db = {
    ...
    async init() {
      ...
      sqlDb.exec(`CREATE TABLE IF NOT EXISTS clicks (id INTEGER PRIMARY KEY AUTOINCREMENT, link_id INTEGER NOT NULL, ip TEXT, lat REAL, lng REAL, address TEXT, client_lat REAL, client_lng REAL, user_agent TEXT, fingerprint TEXT, timestamp TEXT DEFAULT (datetime('now')), FOREIGN KEY (link_id) REFERENCES links(id))`);
      ...
      const cMigs = { client_lat: "ALTER TABLE clicks ADD COLUMN client_lat REAL", client_lng: "ALTER TABLE clicks ADD COLUMN client_lng REAL", fingerprint: "ALTER TABLE clicks ADD COLUMN fingerprint TEXT" };
